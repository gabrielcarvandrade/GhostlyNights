...
public class Player extends Actor 
{
    ...
    private Fantasma fantasmaProximo;
    ...
    private void verificaColisao() 
    {
        ...
        if (tempoColisao == 0) 
        {
            Fantasma inimigo = (Fantasma) getOneIntersectingObject(Fantasma.class);

            if (inimigo != null) 
            {
                hp -= inimigo.obterDanoFantasma();
    ...
    private void inimigoMaisProximo()
    {
        ...
        {
            ArrayList<Fantasma> fantasma = new ArrayList<>(); // declara um ArrayList para a utilizaçao do metodo getNeighbours
            int raioDeDistancia = 100;
            int indice = 0;
            double distancia;
            double menorDistancia = 0;
            // Enquanto o ArrayList for nulo, o programa aumenta o raio de procura
            while (fantasma.size() == 0)
            {
                fantasma.addAll((ArrayList<Fantasma>) getObjectsInRange(raioDeDistancia, Fantasma.class)); // Adiciona ao collection inimigos todos os inimigos a uma distancia de 500 pixels
                raioDeDistancia += 100;
            }
            // Faz o calculo de qual o fantasma com a menor distancia do player
            for (int i = 0; i < fantasma.size(); i++)
            {
                distancia = Math.sqrt(Math.pow(getX()-fantasma.get(i).getX(), 2) + Math.pow(getY()-fantasma.get(i).getY(), 2));
                if (menorDistancia == 0)
            ...
            if (fantasma != null)
                fantasmaProximo = fantasma.get(indice);
        }
    }
    ...
    private void lançaMagiaAutomatico()
    {
        ...
        if(Fantasma.class!=null)
        {
            if (tempo % 30 == 0)
            {
                mundo.addObject(bolaDeFogo, getX(), getY());
                Som.tocarSomBolaDeFogo();
                if (fantasmaProximo != null)
                    bolaDeFogo.turnTowards(fantasmaProximo.getX(), fantasmaProximo.getY()); // faz a bola ir em direçao ao primeiro inimigo detectado
            }
        }
    }
        ...
        if(pontosUlt >= 20)
        {
            if(Fantasma.class!=null)
            {
                if (Greenfoot.isKeyDown("SPACE")) 
                {
                    ...
                    if (fantasmaProximo != null)
                    {
                        ultBolaDeFogo.turnTowards(fantasmaProximo.getX(), fantasmaProximo.getY()); // faz a bola ir em direçao ao primeiro inimigo detectado pelo metodo getNeighbours 
                    }
                    pontosUlt -= 20;
                }
            }
