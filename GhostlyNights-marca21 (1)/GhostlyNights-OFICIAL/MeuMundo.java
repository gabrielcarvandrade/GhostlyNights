...
public class MeuMundo extends World 
{
    ...
    private void prepare() 
    {
        ...
        setPaintOrder(BarraExperiencia.class, Fantasma.class);
        // Cria a barra de HP
    ...
    {
        if (getObjects(Fantasma.class).size() == 0)
            addObject(new Fantasma(player), 0, 0);
        ...
        if (count % cooldownSpawnFantasma == 0) 
        {
            ...
            {
                case 0 : addObject(new Fantasma(player), 0, randomizadorAltura); break;
                case 1 : addObject(new Fantasma(player), randomizadorLargura, 0); break;
                case 2 : addObject(new Fantasma(player), randomizadorLargura, 600); break;
                case 3 : addObject(new Fantasma(player), 800, randomizadorAltura); break;
            }
        }
    }
    ...
    {
        Fantasma fantasma = new Fantasma(player);
        if(fantasma.aumentarSpawnFantasma() == true)
        {
    ...
    private void mostrarPontuaçao()
    {
        ...
        showText("Você perdeu! " + "\n" + "- Voce sobreviveu por " + (player.getTempo()/60) + " segundos" + "\n" 
            ...
            + "- Nivel Inimigo: "+ fantasma.getNivelFantasma() + "\n" + "Aperte R para recomeçar!", 400, 300);
    }
}
